...
async def test_default_agentic_identity_is_used_without_request_metadata():
    ...
    assert token_provider.calls == [("agentic_identity", PUBLIC.agent_bot_scope, None, identity)]
...
@pytest.mark.asyncio
async def test_agentic_identity_requires_named_token_provider_capability():
    calls = []
    ...
    with pytest.raises(ValueError, match="does not implement get_agentic_identity_token"):
        await client.post_resource()
