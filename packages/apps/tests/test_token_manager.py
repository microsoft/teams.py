...
class TestTokenManager:
    ...
    async def test_get_user_token_success(self):
        ...
        # Mock the UserTokenClient
        mock_user_token_client = MagicMock()
        mock_user_token_client.get = AsyncMock(return_value=mock_token_response)

        mock_http_client = MagicMock(spec=Client)
        mock_http_client.clone = MagicMock(return_value=mock_http_client)

        with patch("microsoft.teams.apps.token_manager.UserTokenClient", return_value=mock_user_token_client):
            manager = TokenManager(
                http_client=mock_http_client,
                credentials=mock_credentials,
            ...
            mock_user_token_client.get.assert_called_once()
