...
Each bot exposes:
- `POST /api/messages` — Teams traffic (registered by `FastAPIAdapter` on the
  shared FastAPI app)
...
- **No auth on `/a2a`.** This sample configures no authenticator on
  `DefaultRequestHandler`, so any caller can post a handoff
