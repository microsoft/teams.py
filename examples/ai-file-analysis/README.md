...
Leave any of them unset and the example starts in metadata-only mode. It still receives, downloads, and reports every
...
That shapes how you write a handler rather than being a detail to note. Download when the file arrives, as this sample does, instead of storing an `IncomingFile` to read later. Queued work, retry-with-backoff, and anything that waits on a person are the shapes that run into it.
